...
/* The AEC filter is stored in the time domain. The delay estimator only takes per phase energies, which are
 * independent of element order, so this test works on the stored mantissas directly. */
#define PHASE_AIR_LEN           AEC_FILTER_TD_LENGTH

static void calc_td_frame_energy_fp(double *output, const double *input, int length) {
    *output = 0.0;
...
int estimate_delay_fp(  double h_hat[1][NUM_PHASES_DELAY_EST][PHASE_AIR_LEN], int32_t num_phases, int32_t len_phase,
...
void test_delay_estimate() {
    ...
    double h_hat[1][NUM_PHASES_DELAY_EST][PHASE_AIR_LEN] = {{{0.0}}};
    ...
    for(unsigned ph = 0; ph < num_phases; ph++){
        ...
        TEST_ASSERT_EQUAL_INT32_MESSAGE(length, PHASE_AIR_LEN, "Phase length assumption wrong");
        ...
        int measured_delay_fp = estimate_delay_fp(h_hat, NUM_PHASES_DELAY_EST, PHASE_AIR_LEN,
