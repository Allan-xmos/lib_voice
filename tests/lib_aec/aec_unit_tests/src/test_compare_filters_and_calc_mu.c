...
/* This test only resets and copies filters, never interprets them as impulse responses, so it works on the stored
 * mantissas directly and does not care about the element order. */
#define NUM_TAPS (AEC_FILTER_TD_LENGTH)
...
typedef struct {
    ...
    //common
    //The AEC filters are stored as time domain taps
    double h_hat[TEST_NUM_Y][TEST_NUM_X][TEST_MAIN_PHASES][NUM_TAPS];
    double h_hat_shadow[TEST_NUM_Y][TEST_NUM_X][TEST_SHADOW_PHASES][NUM_TAPS];
    ...
    for(int ch=0; ch<params->x_channels; ch++) {
        for(int ph=0; ph<params->shadow_phases; ph++) {
            for(int tap=0; tap<NUM_TAPS; tap++) {
                params->h_hat_shadow[y_ch][ch][ph][tap] = 0.0;
            }
        }
    }
}
...
void reset_main_filter_fp(shadow_filt_params_fp_t *params, int y_ch) {
    ...
    for(int ch=0; ch<params->x_channels; ch++) {
        for(int ph=0; ph<params->main_phases; ph++) {
            for(int tap=0; tap<NUM_TAPS; tap++) {
                params->h_hat[y_ch][ch][ph][tap] = 0.0;
            }
        }
    }
}
...
void shadow_to_main_filter_copy_fp(shadow_filt_params_fp_t *params, int y_ch) {
    for(int ch=0; ch<params->x_channels; ch++) {
        for(int ph=0; ph<params->shadow_phases; ph++) {
            for(int tap=0; tap<NUM_TAPS; tap++) {
                params->h_hat[y_ch][ch][ph][tap] = params->h_hat_shadow[y_ch][ch][ph][tap];
            }
        }
...
void main_to_shadow_filter_copy_fp(shadow_filt_params_fp_t *params, int y_ch) {
    for(int ch=0; ch<params->x_channels; ch++) {
        for(int ph=0; ph<params->shadow_phases; ph++) {
            for(int tap=0; tap<NUM_TAPS; tap++) {
                params->h_hat_shadow[y_ch][ch][ph][tap] = params->h_hat[y_ch][ch][ph][tap];
            }
        }
    }
}
    ...
    for(int iter=0; iter<(1<<11)/F; iter++) {
        ...
        for(int ych=0; ych<num_y_channels; ych++) {
            ...
            if(params_fp->shadow_flag[ych] != aec_state.main_state.shared_state->shadow_filter_params.shadow_flag[ych]) {
                printf("iter %d. shadow_flag (ref %d, dut %ld) error\n", iter, params_fp->shadow_flag[ych], (long)aec_state.main_state.shared_state->shadow_filter_params.shadow_flag[ych]);
                assert(0);
            }
            ...
            /* Compare h_hat and h_hat_shadow. This function only ever resets filters or copies them wholesale, and
             * both operations are bit preserving in the time domain representation too, so exact agreement with the
             * reference is still required. */
            for(int xch=0; xch<num_x_channels; xch++) {
                for(int ph=0; ph<aec_state.main_state.num_phases; ph++) {
                    unsigned diff_h_hat = vector_int16_maxdiff(aec_state.main_state.h_hat[ych][xch*aec_state.main_state.num_phases + ph].data, aec_state.main_state.h_hat[ych][xch*aec_state.main_state.num_phases + ph].exp, &params_fp->h_hat[ych][xch][ph][0], 0, NUM_TAPS);
                ...
                for(int ph=0; ph<aec_state.shadow_state.num_phases; ph++) {
                    unsigned diff_h_hat_shadow = vector_int16_maxdiff(aec_state.shadow_state.h_hat[ych][xch*aec_state.shadow_state.num_phases + ph].data, aec_state.shadow_state.h_hat[ych][xch*aec_state.shadow_state.num_phases + ph].exp, &params_fp->h_hat_shadow[ych][xch][ph][0], 0, NUM_TAPS);
        ...
        for(int ych=0; ych<num_y_channels; ych++) {
            ...
            if(params_fp->mu_coh_timer[ych] != aec_state.main_state.shared_state->coh_mu_state[ych].mu_coh_timer) {
                printf("iter %d. mu_coh_timer mismatch. (ref %d, dut %ld)\n",iter, params_fp->mu_coh_timer[ych], (long)aec_state.main_state.shared_state->coh_mu_state[ych].mu_coh_timer); assert(0);}

            //compare mu_shad_count
            if(params_fp->mu_shad_count[ych] != aec_state.main_state.shared_state->coh_mu_state[ych].mu_shad_count) {printf("iter %d. mu_shad_count mismatch. (ref %d, dut %ld\n",iter, params_fp->mu_shad_count[ych], (long)aec_state.main_state.shared_state->coh_mu_state[ych].mu_shad_count); assert(0);}
